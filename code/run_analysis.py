...
def main():
    analyze_mocks(set_name='ideal_catwise',
                    overwrite=False, compute_Cells=False, max_mocks_for_Cells=12)
...
def get_fns_to_analyze(dir_mocks, case_dict, max_mocks=None):
    ...
    print(f"analyzing the first {len(fns_mock)}")

    return fns_mock

def analyze_mocks(set_name, overwrite=False, compute_Cells=True, max_mocks_for_Cells=None):
    """
    ...
    dir_results = os.path.join(RESULTDIR, 'results/results_mocks')
    Path.mkdir(Path(dir_results), exist_ok=True, parents=True)

    case_dicts = gm.case_set(set_name=set_name)
    print(f"got {len(case_dicts)} cases")
    ...
    for i, case_dict in enumerate(case_dicts):
        ...
        for i, fn_mock in enumerate(fns_to_analyze):
            ...
            print(f"analyze_mocks(): loaded file {fn_mock}")
            ...
            if not os.path.exists(fn_res) or overwrite:
                ...
                print(f"analyze_mocks(): wrote file {fn_res}")
            ...
            if compute_Cells and fn_mock in fns_to_analyze_Cells:
                ...
                for Lambda in [1e-2, 1e-1]:
                    ...
                    np.save(fn_res, result_dict)
                    print(f"analyze_Cells(): wrote file {fn_res}")
...
def analyze_data(overwrite=False):
    ...
    for [catalog_name, fn_cat, selfunc_mode] in [quaia_settings, catwise_settings]:
        ...
        print(f"analyzing {catalog_name}")
